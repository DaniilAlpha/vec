...
MiunteResult vec_test_init(void) {
    MIUNTE_EXPECT(my_vec_len(&vec) == 0, "new vector is empty");
    MIUNTE_EXPECT(
        my_vec_cap(&vec) == VEC_MIN_CAP,
        "new vector's capacity is VEC_MIN_CAP"
    );
    MIUNTE_EXPECT(my_vec_buf(&vec) != NULL, "new vec's buf isn't NULL");
    MIUNTE_EXPECT(
...
MiunteResult vec_test_access_push_insert_clear(void) {
    ...
    MIUNTE_EXPECT(
        my_vec_len(&vec) == 0 && my_vec_cap(&vec) == VEC_MIN_CAP,
...
MiunteResult vec_test_capacity(void) {
    MIUNTE_EXPECT(
        my_vec_cap(&vec) == VEC_MIN_CAP && my_vec_len(&vec) == 0,
    ...
    MIUNTE_EXPECT(
        my_vec_cap(&vec) == 128 && my_vec_len(&vec) == 100,
    ...
    MIUNTE_EXPECT(
        my_vec_cap(&vec) == 64 && my_vec_len(&vec) == 50,
